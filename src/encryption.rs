...
use chacha20poly1305::{
    XChaCha20Poly1305, Key as ChaChaKey,
};
use hkdf::Hkdf;
use sha2::Sha256;
...
pub enum EncryptionState {
    Uninitialized,
    KeyExchangeInProgress { static_secret: StaticSecret },
    Established {
...
impl EncryptionManager {
    ...
    pub fn complete_key_exchange(&self, peer_public_key: [u8; 32], cipher_suite: CipherSuite) -> Result<(), NfsError> {
        let mut state = self.state.lock().map_err(|_| NfsError::LockError)?;
        ...
        match &*state {
            EncryptionState::KeyExchangeInProgress { static_secret } => {
                ...
                let (encryption_key, decryption_key) = self.derive_keys(shared_secret.as_bytes(), cipher_suite)?;
    ...
    fn derive_keys(&self, shared_secret: &[u8], cipher_suite: CipherSuite) -> Result<(Vec<u8>, Vec<u8>), NfsError> {
    ...
    pub fn encrypt(&self, plaintext: &[u8]) -> Result<Vec<u8>, NfsError> {
        ...
        match &*state {
            EncryptionState::Established { cipher_suite, encryption_key, .. } => {
                match cipher_suite {
                    CipherSuite::XChaCha20Poly1305 => self.encrypt_xchacha20poly1305(plaintext, encryption_key),
    ...
    pub fn decrypt(&self, ciphertext: &[u8]) -> Result<Vec<u8>, NfsError> {
        ...
        match &*state {
            EncryptionState::Established { cipher_suite, decryption_key, .. } => {
                match cipher_suite {
                    CipherSuite::XChaCha20Poly1305 => self.decrypt_xchacha20poly1305(ciphertext, decryption_key),
    ...
    fn decrypt_xchacha20poly1305(&self, ciphertext: &[u8], key: &[u8]) -> Result<Vec<u8>, NfsError> {
        if key.len() != 32 {
            return Err(NfsError::InvalidKeyLength);
        }
    ...
    fn encrypt_aes_gcm(&self, plaintext: &[u8], key: &[u8]) -> Result<Vec<u8>, NfsError> {
        ...
        let cipher = Aes256Gcm::new_from_slice(key)
            .map_err(|_| NfsError::InvalidKeyLength)?;
...
impl Clone for EncryptionManager {
    fn clone(&self) -> Self {
        ...
        let new_state = match &*state {
            ...
            EncryptionState::Established { cipher_suite, encryption_key, decryption_key } => {
