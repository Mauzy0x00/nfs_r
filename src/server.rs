...
impl NfsServer {
    pub fn new(export_path: PathBuf, bind_address: String, keypair: KeyPair) -> Result<Self, NfsError> {
        ...
        loop {
            match connection.receive_message::<NfsMessage>().await {
                Ok(message) => {
                    ...
                    connection.send_message(&NfsResponse::Success(response)).await?;
                }
                Err(e) => {
                    log::error!("Error receiving message from {}: {:?}", peer_addr, e);
                    connection.send_message(&NfsResponse::Error(e.to_string())).await?;
                    break;
                }
            }
        }
    ...
    async fn handle_operation(&self, operation: NfsOperation, client_addr: &std::net::SocketAddr) -> Result<Vec<u8>, NfsError> {
        match operation {
            NfsOperation::Read { path, offset, length } => {
            ...
            NfsOperation::Readdir { path } => {
                ...
                Ok(bincode::encode_to_vec(&entries, bincode::config::standard())?)
            },
            ...
            NfsOperation::Symlink { target, linkpath } => {
                ...
                self.fs_manager.create_symlink(abs_target, abs_linkpath).await?;
                Ok(vec![])
            },
    ...
    fn resolve_path(&self, relative_path: PathBuf) -> Result<PathBuf, NfsError> {
        ...
        if !abs_path.starts_with(&self.export_path) {
            return Err(NfsError::InvalidPath("Path traversal attempt detected".into()));
        }
